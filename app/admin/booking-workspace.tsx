...
export function BookingWorkspace({ view = "dashboard" }: { view?: "dashboard" | "manager" | "form" }) {
  ...
  const totalAdvance = confirmed.reduce((sum, item) => sum + (item.advanceReceived ?? 0), 0);

  if (view === "dashboard") return <main className="adminPage adminDashboardPage">
    <header className="adminHeader dashboardHeader"><div><p className="kicker">Private administration</p><h1>Aishwarya Booking Manager</h1><p className="staffRole">Signed in as {staff.username} · {staff.role === "admin" ? "Administrator" : "Read-only viewer"}</p></div><div className="adminNavUtility"><a href="/">View Website ↗</a><button onClick={logout}>Sign Out</button></div></header>
    <section className="adminDashboardGroups">
      <article className="adminDashboardGroup partyHallDashboard"><p className="kicker">Aishwarya Party Hall</p><h2>Booking Administration</h2><div className="dashboardLinkGrid"><a className="primaryDashboardLink" href="/admin/bookings"><b>Booking Manager</b><span>View, edit and manage confirmed bookings</span></a>{staff.role === "admin" && <a href="/admin/bookings/new"><b>Add Confirmed Booking</b><span>Create a new hall booking</span></a>}<a href="/admin/reports/bookings"><b>Monthly &amp; Yearly Reports</b><span>Month-wise and location-wise booking records</span></a>{staff.role === "admin" && <a href="/admin/financial"><b>Financial Report</b><span>Income, expenses and profit or loss</span></a>}</div></article>
      {staff.role === "admin" && <><article className="adminDashboardGroup ssFoodsDashboard"><p className="kicker">SS Foods</p><h2>Catering Administration</h2><div className="dashboardLinkGrid"><a href="/admin/order-additions"><b>Confirm Orders</b><span>Create and manage addition orders</span></a><a href="/admin/catering-expenses"><b>Expenses</b><span>Record catering expenses</span></a><a href="/admin/catering-expenses/profit-loss"><b>Profit / Loss</b><span>Monthly and yearly catering results</span></a></div></article><article className="adminDashboardGroup businessDashboard"><p className="kicker">Overall Business</p><h2>Combined Reporting</h2><div className="dashboardLinkGrid"><a href="/admin/overall-financial"><b>Combined Financial Report</b><span>Aishwarya Party Hall + SS Foods</span></a></div></article><article className="adminDashboardGroup systemDashboard"><p className="kicker">Administration</p><h2>System Tools</h2><div className="dashboardLinkGrid"><a href="/admin/users"><b>Manage Users</b><span>Administrators and viewers</span></a><a href="/admin/backup"><b>Database Backup</b><span>Import, export and online sync</span></a></div></article></>}
    </section>
  </main>;

  if (view === "form") return <main className="adminPage bookingFormPage">
    <header className="adminHeader"><div><p className="kicker">Aishwarya Party Hall</p><h1>{editing ? "Edit Confirmed Booking" : "Add a Confirmed Booking"}</h1><p className="staffRole">Booking details, customer payment and function schedule</p></div><div className="adminHeaderActions"><a href="/admin/bookings">Booking Manager</a><a href="/admin/reports/bookings">Monthly &amp; Yearly Reports</a><a href="/admin">Admin Home</a></div></header>
  ...
  return <main className="adminPage bookingManagerPage">
    <header className="adminHeader"><div><p className="kicker">Aishwarya Party Hall</p><h1>Booking Manager</h1><p className="staffRole">Confirmed bookings, payments and customer records</p></div><div className="adminHeaderActions">{staff.role === "admin" && <a className="primaryAdminAction" href="/admin/bookings/new">Add Confirmed Booking</a>}<a href="/admin/reports/bookings">Monthly &amp; Yearly Reports</a>{staff.role === "admin" && <a href="/admin/financial">Financial Report</a>}<a href="/admin">Admin Home</a></div></header>
    <section className="bookingManagerSummary"><article><small>Confirmed Bookings</small><b>{confirmed.length}</b></article>{staff.role === "admin" && <><article><small>Total Booking Value</small><b>₹{totalValue.toLocaleString("en-IN")}</b></article><article><small>Advance Received</small><b>₹{totalAdvance.toLocaleString("en-IN")}</b></article><article><small>Balance Pending</small><b>₹{(totalValue - totalAdvance).toLocaleString("en-IN")}</b></article></>}</section>
    <section className="bookingReport"><div className="reportHead"><div><p className="kicker">Confirmed booking register</p><h2>{staff.role === "admin" ? "Bookings & payment summary" : "Booking details"}</h2><p>All confirmed bookings across Padi and Korattur</p></div><div className="reportHeadActions"><button className="whatsappReportButton withoutAmount" disabled={sharingReport} onClick={() => sendUpcomingReport(false)}>{sharingReport ? "Preparing…" : "WhatsApp Without Amount"}</button><button className="whatsappReportButton" disabled={sharingReport} onClick={() => sendUpcomingReport(true)}>{sharingReport ? "Preparing…" : "WhatsApp With Amount"}</button><button onClick={() => window.print()}>Print report</button></div></div><div className="reportTableWrap"><table><thead><tr><th>Bill No.</th><th className="dateColumn">Date &amp; time</th><th>Location</th><th>Function name</th><th>Customer details</th>{staff.role === "admin" && <><th className="moneyColumn">Amount</th><th className="moneyColumn">Advance</th><th className="moneyColumn">Balance</th><th>Actions</th></>}</tr></thead><tbody>{confirmed.map((item) => { const balance = (item.amount ?? 0) - (item.advanceReceived ?? 0); return <tr key={item.id}><td className="billCell">{item.billNo || "—"}</td><td className="dateCell"><b>{new Date(`${item.bookingDate}T00:00:00`).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</b><small>{formatTimeRange12Hour(item.startTime, item.endTime)}</small></td><td><span className={`locationBadge ${item.location.toLowerCase()}`}>{item.location}</span></td><td className="functionCell">{item.functionName}</td><td className="customerCell"><b>{item.customerName}</b><a href={`tel:${item.mobile}`}>{item.mobile}</a></td>{staff.role === "admin" && <><td className="moneyCell">₹{(item.amount ?? 0).toLocaleString("en-IN")}</td><td className="moneyCell advanceCell">₹{(item.advanceReceived ?? 0).toLocaleString("en-IN")}</td><td className="moneyCell balanceCell"><b>₹{balance.toLocaleString("en-IN")}</b></td><td><div className="bookingActions"><a href={`/admin/bookings/new?editBooking=${item.id}`}>Edit</a><button onClick={() => cancel(item)}>Delete</button></div></td></>}</tr>; })}{!confirmed.length && <tr><td colSpan={staff.role === "admin" ? 9 : 5}>No confirmed bookings found.</td></tr>}</tbody></table></div>{message && <p className="adminMessage">{message}</p>}</section>
