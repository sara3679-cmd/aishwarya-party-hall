...
export function StaffRentWorkspace({ kind }: { kind: 'Salary' | 'Rent' }) {
  ...
  const canFinalise = `${month}-${monthEnd}` <= date;
  ...
  async function downloadSalarySlipImage(bill: Bill, openWhatsApp = false) {
    ...
    canvas.width = 1200; canvas.height = 1420;
    ...
    ctx.font = '700 58px Georgia'; ctx.fillText('Salary Slip', 76, 155);
    ...
    const rows = [['Monthly salary', money(bill.basic)], ['Unpaid leave', `${unpaidLeaveDays(data, bill.entity, bill.month)} day(s) · −${money(bill.leave)}`], ['Balance advance from previous month', money(previousAdvance)], ['Advance recovery', `−${money(bill.recovery)}`], ['Other additions', `+${money(bill.extra)}`], ['Balance to pay', money(bill.total)], ['Paid', money(paid(bill))], ['Balance amount', money(balance(bill))]];
    let y = 535;
    for (const [label, amount] of rows) {
      const isTotal = label === 'Balance to pay' || label === 'Balance amount';
      if (isTotal) { ctx.fillStyle = '#fff2d7'; ctx.fillRect(60, y - 43, 1080, 76); }
      ctx.fillStyle = isTotal ? '#830b12' : '#3f322d'; ctx.font = `${isTotal ? '700' : '600'} 26px Arial`; ctx.fillText(label, 90, y);
    ...
    ctx.fillStyle = '#830b12'; ctx.fillRect(60, 1285, 1080, 88); ctx.fillStyle = '#ffffff'; ctx.font = '700 24px Arial'; ctx.textAlign = 'center'; ctx.fillText(balance(bill) === 0 ? 'PAYMENT SETTLED' : 'PAYMENT PENDING', 600, 1339); ctx.textAlign = 'left';
  ...
  </div>{slip && <div className="sr-slip"><div className="sr-screen sr-toolbar"><button onClick={() => downloadSalarySlipImage(slip, true)}>Download & Open WhatsApp</button><button className="sr-text-button" onClick={() => downloadSalarySlipImage(slip)}>Download bill only</button><button onClick={() => window.print()}>Print / Save PDF</button><button onClick={() => setSlip(null)}>Close</button></div><header className="salary-slip-brand"><img src="/images/brand/aishwarya-party-hall-logo.jpg" alt="Aishwarya Party Hall logo"/><div><p>AISHWARYA PARTY HALL</p><h1>Salary Slip</h1><span>{slip.location} · Chennai</span></div><b>{slip.month}</b></header><section className="salary-slip-person"><div><span>STAFF MEMBER</span><h2>{slip.name}</h2></div><div><span>PAY PERIOD</span><b>{slip.month}</b></div></section><dl><div><dt>Monthly salary</dt><dd>{money(slip.basic)}</dd></div><div><dt>Unpaid leave</dt><dd>{unpaidLeaveDays(data, slip.entity, slip.month)} day(s) · −{money(slip.leave)}</dd></div><div><dt>Balance advance from previous month</dt><dd>{money(slipPreviousAdvance)}</dd></div><div><dt>Advance recovery</dt><dd>−{money(slip.recovery)}</dd></div><div><dt>Other additions</dt><dd>+{money(slip.extra)}</dd></div><div className="net"><dt>Balance to pay</dt><dd>{money(slip.total)}</dd></div><div><dt>Paid</dt><dd>{money(paid(slip))}</dd></div><div className="balance-line"><dt>Balance amount</dt><dd>{money(balance(slip))}</dd></div></dl><footer className="salary-slip-footer"><b>{balance(slip) === 0 ? 'PAYMENT SETTLED' : 'PAYMENT PENDING'}</b><span>Generated {displayDate(date)} · Aishwarya Party Hall · Padi & Korattur</span></footer></div>}</main>;
}
