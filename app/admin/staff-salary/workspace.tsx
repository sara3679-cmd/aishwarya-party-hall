...
export function StaffRentWorkspace({ kind }: { kind: 'Salary' | 'Rent' }) {
  ...
  async function downloadSalarySlipImage(bill: Bill, openWhatsApp = false) {
    ...
    ctx.fillStyle = '#830b12'; ctx.fillRect(60, 1285, 1080, 88); ctx.fillStyle = '#ffffff'; ctx.font = '700 24px Arial'; ctx.textAlign = 'center'; ctx.fillText(balance(bill) === 0 ? 'PAYMENT SETTLED · பணம் செலுத்தப்பட்டது' : 'PAYMENT PENDING · பணம் நிலுவையில் உள்ளது', 600, 1339); ctx.textAlign = 'left';
