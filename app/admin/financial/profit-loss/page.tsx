...
export default function MonthlyYearlyProfitLossPage() {
  ...
  return <main className="adminPage profitLossPage"><header className="adminHeader"><div><p className="kicker">Monthly and yearly statement</p><h1>Profit & Loss Summary</h1><p className="staffRole">Total revenue − expenses</p></div><div className="adminHeaderActions bookingAdminNav"><a href="/admin/bookings/new">Hall Booking</a><a href="/admin/bookings">Booking Manager</a><a href="/admin/financial">New Expenses &amp; Revenue</a><a href="/admin/reports/bookings">Expenses &amp; Revenue</a><a className="currentNavLink" href="/admin/financial/profit-loss">Profit &amp; Loss</a><a href="/admin">Admin Home</a><button onClick={() => window.print()}>Print Report</button></div></header>
    ...
    <section className="financialCards yearlyCards"><article><small>Yearly booking revenue</small><b>{money(yearly.revenue)}</b></article><article><small>Yearly additional revenue</small><b>{money(yearly.commission)}</b></article><article><small>Yearly total revenue</small><b>{money(yearly.income)}</b></article><article><small>Yearly expenses</small><b>{money(yearly.expenses)}</b></article><article className={yearly.profit >= 0 ? "profitCard" : "lossCard"}><small>{yearly.profit >= 0 ? "Yearly net profit" : "Yearly net loss"}</small><b>{money(yearly.profit)}</b></article></section>
    <section className="financeTable monthlyProfitTable"><h2>{year} Monthly Profit & Loss</h2><div className="reportTableWrap"><table><thead><tr><th>Month</th><th>Booking revenue</th><th>Additional revenue</th><th>Total revenue</th><th>Advance collected</th><th>Outstanding</th><th>Expenses</th><th>Profit / Loss</th></tr></thead><tbody>{months.map((row, index) => <tr key={row.month}><td><a style={{textDecoration:"underline",textUnderlineOffset:"3px",fontWeight:700}} href={`/admin/financial/profit-loss/month?month=${year}-${String(index + 1).padStart(2,"0")}&location=${encodeURIComponent(location)}`}>{row.month}</a></td><td>{money(row.revenue)}</td><td>{money(row.commission)}</td><td>{money(row.income)}</td><td>{money(row.collected)}</td><td>{money(row.outstanding)}</td><td>{money(row.expenses)}</td><td className={row.profit >= 0 ? "profitAmount" : "lossAmount"}>{row.profit >= 0 ? "Profit " : "Loss "}{money(row.profit)}</td></tr>)}</tbody><tfoot><tr><th>Yearly total</th><th>{money(yearly.revenue)}</th><th>{money(yearly.commission)}</th><th>{money(yearly.income)}</th><th>{money(yearly.collected)}</th><th>{money(yearly.outstanding)}</th><th>{money(yearly.expenses)}</th><th className={yearly.profit >= 0 ? "profitAmount" : "lossAmount"}>{yearly.profit >= 0 ? "Profit " : "Loss "}{money(yearly.profit)}</th></tr></tfoot></table></div></section>
    <p className="financeNote"><b>Calculation:</b> Booking revenue + Additional revenue details − Expense details. Monthly and yearly totals follow the selected year and location.</p>
  </main>;
}
