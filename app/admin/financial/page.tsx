...
export default function FinancialReportPage() {
  ...
  async function addIncome(event: FormEvent<HTMLFormElement>) {
    ...
    const data = await response.json();
    if (!response.ok) { setMessage(data.error); return; }
    form.reset(); setMessage("Revenue saved."); load();
  }
  ...
  return <main className="adminPage financialPage"><header className="adminHeader"><div><p className="kicker">Private financial administration</p><h1>Profit & Loss Report</h1></div><div className="adminHeaderActions bookingAdminNav"><a href="/admin/bookings/new">New Booking</a><a href="/admin/bookings">Booking Manager</a><a href="/admin/reports/bookings">Expenses &amp; Revenue</a><a className="currentNavLink" href="/admin/financial/profit-loss">Profit &amp; Loss</a><a href="/admin">Admin Home</a><button onClick={() => window.print()}>Print Report</button></div></header>
    ...
    </section><section className="incomeSection"><form className="adminForm incomeForm" onSubmit={addIncome}><h2>Add revenue</h2><div className="formRow"><label>Revenue date<input name="incomeDate" type="date" required /></label><label>Location<select name="location"><option>Padi</option><option>Korattur</option><option>General</option></select></label></div><label>Revenue source<select name="category"><option value="Stage Commission">Stage Revenue</option><option value="Photographer Commission">Photographer Revenue</option><option value="Other Income">Other Revenue</option></select></label><label>Description<input name="description" required placeholder="Customer, event or revenue details" /></label><label>Amount received (₹)<input name="amount" type="number" min="1" required /></label><button>Save revenue</button></form><div className="financeTable"><h2>Revenue details</h2><div className="reportTableWrap"><table><thead><tr><th>Date</th><th>Location</th><th>Revenue source</th><th>Description</th><th>Amount</th><th></th></tr></thead><tbody>{income.map((item) => <tr key={item.id}><td>{item.incomeDate}</td><td>{item.location}</td><td>{item.category.replace(/Commission/gi, "Revenue").replace(/Income/gi, "Revenue")}</td><td>{item.description.replace(/Commission/gi, "Revenue")}</td><td>{money(item.amount)}</td><td><button onClick={() => removeIncome(item.id)}>Delete</button></td></tr>)}</tbody></table></div></div></section><p className="financeNote">Profit or loss is calculated as total booking and additional revenue, minus recorded expenses for the selected period.</p></main>;
}
