...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [booking] = await db.update(bookings).set({ location: location as "Padi" | "Korattur", bookingDate, startTime, endTime, billNo, functionName, customerName, mobile, amount: Math.round(amount), advanceReceived: Math.round(advanceReceived) }).where(eq(bookings.id, bookingId)).returning();
  return Response.json({ booking });
}
