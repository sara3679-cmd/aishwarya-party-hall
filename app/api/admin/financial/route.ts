...
export async function PUT(request: Request) {
  ...
  const db = getDb();
  const result = await db.insert(additionalIncome).values({ incomeDate, location: location as "Padi" | "Korattur" | "General", category, description, amount: Math.round(amount) });
  const [income] = await db.select().from(additionalIncome).where(eq(additionalIncome.id, Number(result[0].insertId))).limit(1);
