...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [expense] = await getDb().update(expenses).set({ expenseDate, location: location as "Padi" | "Korattur" | "General", category, description, amount: Math.round(amount) }).where(eq(expenses.id, expenseId)).returning();
