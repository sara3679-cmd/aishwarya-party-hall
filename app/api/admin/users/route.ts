...
export async function POST(request: Request) {
  ...
  const [user] = await db.insert(staffUsers).values({ username, passwordSalt: salt, passwordHash: await hashPassword(password, salt), role: role as "admin" | "viewer" }).returning({ id: staffUsers.id, username: staffUsers.username, role: staffUsers.role, createdAt: staffUsers.createdAt });
