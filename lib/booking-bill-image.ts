...
export type CustomerBillBooking = { location: string; bookingDate: string; startTime: string; endTime: string; billNo: string; functionName: string; customerName: string; mobile: string; mobile2?: string; address?: string; createdAt?: string; amount?: number; advanceReceived?: number; cctvPassword?: string };
...
export async function createCustomerBill(booking: CustomerBillBooking): Promise<File> {
  ...
  const groups = [
    ...
    ...(booking.location === "Padi" ? [{ title: "Live CCTV Access", fields: [
      ["Temporary username", booking.billNo],
      ["Temporary password", booking.cctvPassword || "Pending"],
      ["Customer login", "www.aishwaryapartyhall.in/cctv"],
      ["Access period", "Your booked event time only"],
    ] }] : []),
  ];
  ctx.font = "28px Arial";
