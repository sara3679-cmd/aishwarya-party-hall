...
const records={version:1,employees:[employee],halls:[],entries:[{id:'a',employee:'e1',date:'2026-09-01',kind:'Advance',amount:5000,note:''},{id:'l',employee:'e1',date:'2026-09-02',kind:'Unpaid leave',amount:0.5,note:''}],bills:[]};
test('half-day deduction, advance recovery and net salary',()=>{const b=salaryBill(records,employee,'2026-09');assert.equal(b.leave,500);assert.equal(b.recovery,2000);assert.equal(b.total,27500);});
test('finalised recovery is reserved once and partial payment stays due',()=>{const b=salaryBill(records,employee,'2026-09');const next={...records,bills:[b]};assert.equal(advanceBalance(next,'e1'),3000);b.payments.push({id:'p',date:'2026-09-30',amount:10000,method:'Cash',reference:''});assert.equal(balance(b),17500);b.payments.push({id:'p2',date:'2026-09-30',amount:17500,method:'Cash',reference:''});assert.equal(balance(b),0);assert.equal(advanceBalance(next,'e1'),3000);});
