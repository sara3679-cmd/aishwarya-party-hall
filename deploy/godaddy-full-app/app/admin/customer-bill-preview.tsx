...
export function CustomerBillPreview({ booking, onClose }: { booking: CustomerBillBooking; onClose: () => void }) {
  ...
  function send() {
    try {
      ...
      const cctvMessage = booking.location === "Padi" ? ` Live CCTV username: ${booking.billNo}. Temporary password: ${booking.cctvPassword || "Pending"}. Sign in at www.aishwaryapartyhall.in/cctv during your booked event time.` : "";
      window.open(`https://wa.me/${number}?text=${encodeURIComponent(`Hello ${booking.customerName}, your Aishwarya Party Hall – ${booking.location} booking bill (${booking.billNo}).${cctvMessage}`)}`, "_blank", "noopener,noreferrer");
  ...
  return <dialog ref={dialog} className="customerBillDialog" onCancel={onClose} aria-labelledby="customer-bill-title">
    ...
    {booking.location === "Padi" && <p>Live CCTV username: <b>{booking.billNo}</b> · Temporary password: <b>{booking.cctvPassword || "Pending"}</b></p>}
