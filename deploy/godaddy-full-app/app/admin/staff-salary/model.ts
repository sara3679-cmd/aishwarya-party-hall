export type Employee = { id: string; name: string; location: string; phone: string; role: string; joined: string; salary: number; recovery: number; paidLeave: number };
...
export function advanceBalance(data: Records, employee: string, through = '9999-12-31') {
  ...
  const recovered = data.bills.filter(b => b.entity === employee && b.kind === 'Salary').reduce((s, b) => s + b.recovery, 0);
...
export function salaryBill(data: Records, e: Employee, month: string): Bill {
  ...
  const leave = round(e.salary / days * Math.min(activeDays, sum('Unpaid leave')));
  const debit = sum('Debit');
  const extra = sum('Bonus / overtime') + sum('Reimbursement');
  const available = Math.max(0, basic - leave - debit + extra);
  const recovery = Math.min(e.recovery, advanceBalance(data, e.id, `${month}-${days}`), available);
...
export function validRecords(value: unknown): value is Records {
  ...
  try {
    return v.version === 1 && [v.employees,v.entries,v.halls,v.bills].every(Array.isArray)
      && v.employees.every(e => [e.id,e.name,e.phone,e.role,e.location].every(text) && date(e.joined) && [e.salary,e.recovery,e.paidLeave].every(number))
      ...
      && v.entries.every(e => [e.id,e.employee,e.note].every(text) && date(e.date) && number(e.amount) && ['Advance','Debit','Paid leave','Unpaid leave','Bonus / overtime','Reimbursement'].includes(e.kind) && v.employees.some(x=>x.id===e.employee))
